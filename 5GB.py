...
# DBTITLE 1,chạy 22 câu
import time
from pyspark.sql import SparkSession

# Khởi động Spark
spark = SparkSession.builder \
    .appName("TPCH truy van 5GB") \
    .getOrCreate()
...
# DBTITLE 1,Test cau 21

# TRUY VẤN 21
result_q21 = spark.sql("""
select
	s_acctbal,
	s_name,
	n_name,
...
order by
	...
	p_partkey;
""")
result_q21.show(truncate=False)
...
# 1 Đặt tên cột cho bảng lineitem
...
# DBTITLE 1,Tạo session read 5GB
from pyspark.sql import SparkSession

# Khởi tạo Spark session
spark = SparkSession.builder \
    .appName("TPC-H 5GB again") \
    .getOrCreate()

# Đọc dữ liệu từ các file với inferSchema=True
orders_df = spark.read.csv('dbfs:/FileStore/shared_uploads/5GB/orders.tbl', sep='|', header=False, inferSchema=True)
lineitem_df = spark.read.csv('dbfs:/FileStore/shared_uploads/5GB/lineitem.tbl', sep='|', header=False, inferSchema=True)
part_df = spark.read.csv('dbfs:/FileStore/shared_uploads/5GB/part.tbl', sep='|', header=False, inferSchema=True)
supplier_df = spark.read.csv('dbfs:/FileStore/shared_uploads/5GB/supplier.tbl', sep='|', header=False, inferSchema=True)
customer_df = spark.read.csv('dbfs:/FileStore/shared_uploads/5GB/customer.tbl', sep='|', header=False, inferSchema=True)
nation_df = spark.read.csv('dbfs:/FileStore/shared_uploads/5GB/nation.tbl', sep='|', header=False, inferSchema=True)
region_df = spark.read.csv('dbfs:/FileStore/shared_uploads/5GB/region.tbl', sep='|', header=False, inferSchema=True)
partsupp_df = spark.read.csv('dbfs:/FileStore/shared_uploads/5GB/partsupp.tbl', sep='|', header=False, inferSchema=True)
